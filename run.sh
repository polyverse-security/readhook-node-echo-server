...
docker run --privileged --cap-add=SYS_PTRACE --security-opt seccomp=unconfined -it --rm --name readhook-node-echo-server -v $HOOK_PATH:/mnt/cross -v $GLIBC_PATH:/mnt/glibc --entrypoint=/bin/bash polyverse/readhook-node-echo-server
